...
class Core(component.Component):
    ...
    @export
    def get_torrents_status_v2(self, keys, update=False):
        results = []
        torrent_ids = list(self.torrentmanager.torrents)
        for torrent_id in torrent_ids[:]:
            results.append(self.torrentmanager[torrent_id].get_status(keys, update=update))
